...
namespace Tichu.Core.Tests.Bench
{
    ...
    public static class CallNetTrainer
    {
        ...
        /// <summary>
        /// rounds 라운드 self-play. Small Tichu 는 교환 후 14장이라, Grand(×4)+Exchange(×4)를
        /// 수동 드라이브해 Play 진입(4좌석 14장·플레이 전) 시점에 스냅샷 → 완주 → FinishOrder==1 라벨.
        /// </summary>
        public static (float[][] X, int[] y) GenerateSmallData(int rounds, ulong baseSeed)
        {
            var X = new float[rounds * 4][];
            var y = new int[rounds * 4];
            for (int r = 0; r < rounds; r++)
            {
                ...
                // Grand + Exchange 드라이브(Play 진입까지). GameDriver 셋업 순서와 동일.
                int grandNext = 0, exchangeNext = 0, steps = 0;
                while (s.Phase != RoundPhase.Play)
                {
                    if (++steps > 100) throw new InvalidOperationException("small setup stuck");
                    if (s.Phase == RoundPhase.GrandTichuDecision)
                    {
                        int seat = grandNext++;
                        bool call = agents[seat].CallGrandTichu(new DecisionContext(s, seat));
                        ApplySetup(s, call ? GameAction.CallGrandTichu(seat) : GameAction.DeclineGrandTichu(seat));
                    }
                    else if (s.Phase == RoundPhase.Exchange)
                    {
                        int seat = exchangeNext++;
                        var ex = agents[seat].ChooseExchange(new DecisionContext(s, seat));
                        ApplySetup(s, GameAction.Exchange(seat, new[] { ex.ToLeft }, new[] { ex.ToPartner }, new[] { ex.ToRight }));
                    }
                    else break;
                }

                // Play 진입: 14장 스냅샷(플레이 전).
                var feats = new float[4][];
                for (int seat = 0; seat < 4; seat++)
                    feats[seat] = SmallTichuFeatures.Encode(s.Seats[seat].Hand);
                var outcome = new GameDriver(agents).RunRound(s);
                for (int seat = 0; seat < 4; seat++)
                {
                    X[r * 4 + seat] = feats[seat];
        ...
        private static void ApplySetup(GameState s, GameAction a)
        {
            var res = GameEngine.Apply(s, a);
            if (!res.Ok) throw new InvalidOperationException($"setup illegal {a.Kind} seat {a.Seat}: {res.RejectReason}");
        }

        /// <summary>로지스틱 회귀 SGD(이진 크로스엔트로피 + L2). 마지막 10%는 검증셋. dim=피처 길이(Grand/Small 공용).</summary>
        ...
        /// <summary>가중치를 {className}.g.cs 소스로 직렬화(양 미러에 붙여넣을 원문). Grand/Small 공용.</summary>
        public static string EmitWeightsSource(double[] w, double b, double threshold, string className = "GrandTichuWeights")
        {
            var sb = new StringBuilder();
            sb.AppendLine($"// <auto-generated> 콜 헤드 가중치({className}). CallNetTrainer 가 생성. 손으로 편집 금지. </auto-generated>");
